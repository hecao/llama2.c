...
// 填充transformerweights中的指针，顺序按照export指定的来
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    fprintf(stdout, "read from checkpoint %s\n", checkpoint);
...
// 流程可以参考 https://blog.csdn.net/v_JULY_v/article/details/130090649
// 0. 获取embedding 
// 1. rmsnorm
// 2. K Q V 和 WQ WK WV矩阵
float* forward(Transformer* transformer, int token, int pos) {
    ...
    // final rmsnorm
    rmsnorm(x, x, w->rms_final_weight, dim);        // N个Transformer Block 之后的norm

    // classifier into logits
    matmul(s->logits, x, w->wcls, p->dim, p->vocab_size);   // 最后一个线性层
    return s->logits;
}
...
int sample(Sampler* sampler, float* logits) {
    ...
    if (sampler->temperature == 0.0f) {
        ...
        for (int q=0; q<sampler->vocab_size; q++) { logits[q] /= sampler->temperature; }    // 低temperature使输出概率更集中，
